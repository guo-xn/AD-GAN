...
class dataset_loader(torch.utils.data.Dataset):
    def __init__(self, args, dl_mode):
        ...
        att_list = open(self.args.label_dir, 'r', encoding='utf-8').readlines()[0].split()
        atts = [att_list.index(att) + 1 for att in self.args.attrs]
        self.all_images_name = np.loadtxt(self.args.label_dir, skiprows=1, usecols=[0], dtype=np.str)
        self.all_labels_attr = np.loadtxt(self.args.label_dir, skiprows=1, usecols=atts, dtype=np.int)
        ...
        if self.dl_mode == 'train':
            ...
            att_list = open(self.args.test_label_dir, 'r', encoding='utf-8').readlines()[0].split()
            atts = [att_list.index(att) + 1 for att in self.args.attrs]
            self.images_name = np.loadtxt(self.args.test_label_dir, skiprows=1, usecols=[0], dtype=np.str)
            self.labels_attr = np.loadtxt(self.args.test_label_dir, skiprows=1, usecols=atts, dtype=np.int)
    ...
    def __getitem__(self, index):
        ...
        if self.dl_mode == 'test':
            mask = Image.open(self.masks[index % 2000])
        else:
            ld_mask = Image.open(self.masks[index % 2000])
